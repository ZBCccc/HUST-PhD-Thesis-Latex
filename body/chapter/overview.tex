...
针对时间序列预测建模任务下SVM的参数选择与模型选择问题，基于SVM收敛快、参数调整成本低等特点，Bao and Liu~\cite{baoFast2006}提出了一种基于快速网格搜索算法的SVM模型选择算法。随后，Bao et al.~\cite{baoPSO2013}提出了基于粒子群优化（Particle swarm optimization，PSO）与模式搜索（Pattern search，PS）相结合的文化基因进化算法（Memetic algorithm，MA），并将其应用在SVM模型选择优化中。Hu et al.~\cite{huShortterm2016}利用PSO搜索SVM参数提出了一种基于PSO-SVM的预测模型。Hu et al.~\cite{huElectricity2013}提出了基于萤火虫算法（Firefly algorithm，FA）与PS算法相结合的MA算法，以此解决SVM电力负荷预测模型的模型选择问题。
...
基于对时间序列深度学习预测技术神经网络结构选择问题的上述分析，且隐藏结构在深度神经网络中权重参数与结构参数占比最高，后续内容聚焦神经网络隐藏结构选择；输出结构选择与多输出选择合并讨论，输入结构选择与特征选择合并讨论。
...
因此，研究目标是构建高效稳定的时间序列深度学习预测模型隐藏结构构造理论与选择策略。
% \clearpage
...
因此，研究重点延伸至时间序列深度学习预测建模的多输入选择问题，以发展高效优化多输入特征结构的方法。
...
随后，Wang and Li\cite{wang2017stochastic}提出了一种随机配置网络（Stochastic configuration network，SCN），通过迭代增加隐藏层神经元、基于监督机制闭式挑选神经元隐藏层权重并全局更新输出层权重，构造出增长的SMLP模型，并证明了SCN的普适逼近性质（Universal approximation property）。该类改进后的SMLP模型可获得具备理论保证的拟合收敛性与稳定性。
...
随后，Li et al.~\cite{liPSObased2019}建立了一个多循环池结构的ESN预测模型，通过PSO和奇异值分解对循环池权重进行训练和优化后再依次将其加入ESN网络结构中，进一步提升了ESN的预测表现。
...
因此，针对现有深度学习预测建模技术与随机映射方法的不足，研究路径是在相关工作基础上设计SDNN预测模型的新构造方法，削弱传统迭代训练DNN的低效性并缓解随机映射在DNN结构上的不稳定性，同时建立与预测任务和模型相适应的模型选择算法，形成具备高效自适性、理论创新性和应用价值的SDNN时间序列建模技术。
