...
\section{布隆过滤器相关 \label{sec:esm.cnn}}

\subsection{布隆过滤器}
布隆过滤器（Bloom Filter）是一种空间效率高的概率型数据结构，用于判断一个元素是否属于一个集合。
它通过将元素映射到一个固定长度的位向量（Bit Vector）上，利用多个哈希函数将元素映射到不同的位上，从而实现对元素的快速判断。
布隆过滤器的主要操作包括插入元素和查询元素是否存在。
当一个元素被插入布隆过滤器时，它会被多个哈希函数映射到不同的位上，将这些位设置为1。
当查询一个元素是否存在时，它会被多个哈希函数映射到不同的位上，只要有一个位为0，就可以确定该元素不在集合中；只有当所有位都为1时，才可以确定该元素可能在集合中。
