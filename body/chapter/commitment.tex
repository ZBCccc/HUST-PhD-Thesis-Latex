...
第\ref{chap:bf}章的 $\Pi_{\mathsf{VQ}}$ 机制在 $\mathsf{QTree}$ 上为资格检验建立了路径级认证。该机制保证的对象是位值真实性：若服务器给出的 $(a,b,\pi_a)$ 通过 $\mathsf{VfPath}$ 验证，则 $b$ 必然是版本根 $R_X^{(t)}$ 下地址 $a$ 的真实位值。然而，该保证的信任边界未能涵盖对地址归属正确性的检查：即地址 $a$ 是否确实属于当前查询请求所采样的交叉标签子集。第\ref{chap:bf}章的定义 2 已将“地址归属正确”作为绑定健全性的显式前提条件，且命题~\ref{prop:vq-sound} 的安全归约正是建立在该前提基础之上。本节将进一步剖析，为何在 Nomos OPRF 盲化架构下，该前提无法由客户端自行验证，从而暴露出真实的攻击面。

Nomos 的令牌生成协议（算法~\ref{alg:sse-gentoken}）通过 OPRF 盲化保护查询隐私：客户端在令牌生成阶段仅持有盲化的 $\mathsf{xtoken}$ 材料和采样位置 $\{\beta_t\}$，而不持有可直接计算交叉标签的密钥 $K_X$。服务器在搜索阶段使用 $\alpha_j$、$\rho_j$ 与 $\mathsf{xtoken}_{i,j}^{(t)}$ 恢复采样地址
...
\end{equation}
但客户端无法独立重算这些地址。$\mathsf{QTree}$ 路径验证 $\mathsf{VfPath}(R_X^{(t)},a,b,\pi_a)$ 仅检查三元组 $(a,b,\pi_a)$ 的内部一致性——叶子摘要沿路径 $\pi_a$ 是否归约到根 $R_X^{(t)}$——而不检查地址 $a$ 是否属于令牌绑定集合 $\mathcal{A}_{i,j}^{(Q)}$。恶意服务器因此可以保留真实的 $\mathsf{QTree}$ 路径结构，同时把某个应被验证的地址替换为集合外的无关地址 $a^\star$。只要 $a^\star$ 在当前版本的 $\mathsf{QTree}$ 中确实存在且位值满足服务器期望的逻辑判定，路径验证仍会通过。

以下给出一个具体的地址替换攻击（Address Substitution Attack）策略。设候选 $(j,u)$ 的真实判定为 $v_{j,u}^{(Q)}=0$，即存在某个采样地址 $a_0\in\mathcal{A}_{j,u}^{(Q)}$ 满足 $\mathbf{B}^{(t)}[a_0]=0$。恶意服务器选择一个集合外地址 $a^\star\notin\mathcal{A}_{j,u}^{(Q)}$，使得 $\mathbf{B}^{(t)}[a^\star]=1$——在 RBF 位数组中，多数地址的位值为 1，因此这样的 $a^\star$ 容易找到。服务器在 Negative 证据中用 $(a^\star,1,\pi_{a^\star})$ 替换 $(a_0,0,\pi_{a_0})$，并将声明判定位篡改为 $\widehat{v}_{j,u}=1$。由于 $a^\star$ 在 $\mathsf{QTree}$ 中确实存在且 $\mathbf{B}^{(t)}[a^\star]=1$，路径验证 $\mathsf{VfPath}(R_X^{(t)},a^\star,1,\pi_{a^\star})=1$ 成立。客户端在不知道 $\mathcal{A}_{j,u}^{(Q)}$ 的真实内容的情况下，无法检测到地址替换，从而接受伪造的 Positive 判定。该攻击使不满足资格条件的候选通过检验，破坏了资格检验的健全性。
