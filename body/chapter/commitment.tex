...
定义~\ref{def:addr-binding} 与第\ref{chap:bf}章的定义~\ref{def:vq-binding-soundness} 构成互补关系。定义~\ref{def:vq-binding-soundness} 保证``若服务器确实在正确地址上生成证据，则位值不可伪造''；定义~\ref{def:addr-binding} 保证``服务器不能把证明地址替换到认证集合之外''。两项性质共同闭合资格检验的正确性链条。该定义与标准 Merkle 承诺健全性的区别在于承诺对象：标准 Merkle 承诺保证``叶子值不可篡改''，而地址绑定健全性保证``证明所用的地址集合不可替换''。前者是后者的构造工具——Merkle 路径验证用于实现地址归属的可验证性——但安全目标不同。

在上述安全目标的约束下，Nomos 原始搜索令牌仅支持服务器恢复 $k$ 个采样位置对应的交叉标签，而不支持在搜索阶段额外恢复完整的 $\ell$ 个地址。若地址绑定机制要求服务器返回完整集合 $\mathcal{F}_{j,u}$，则令牌语法、搜索代数结构和通信复杂度都将偏离第\ref{chap:bf}章的基础协议。地址绑定机制因此必须满足下述一致性约束：更新阶段可以对完整集合 $\mathcal{F}_{j,u}$ 建立认证，搜索阶段只能对采样集合 $\mathcal{A}_{j,u}^{(Q)}$ 返回证明。
...
三步检查的正确性可从以下角度理解。第一步的签名验证保证根 $\mathsf{rt}_{j,u}$ 由 $\mathcal{G}$ 在某次更新中输出，排除了服务器伪造根认证对象的可能。第二步的 Merkle 路径验证保证每个开封的交叉标签 $\mathsf{xtag}_{j,u}^{(\beta_t)}$ 确实是根 $\mathsf{rt}_{j,u}$ 在位置 $\beta_t$ 上的真实叶子值，排除了服务器用集合外地址冒充采样地址的可能。第三步的地址归属检查保证 $\mathsf{QTree}$ 证明中实际使用的地址与已开封的采样集合一致，将 Merkle 开封的保证传递到 $\mathsf{QTree}$ 路径验证。三步检查的组合效果是：客户端接受的每个 $\mathsf{QTree}$ 证明地址都经过 $\mathcal{G}$ 认证的 Merkle 树验证，从而闭合了第\ref{chap:bf}章定义~\ref{def:vq-binding-soundness} 中``地址归属正确''的前提条件。
