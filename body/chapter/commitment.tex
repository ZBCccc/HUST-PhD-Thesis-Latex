...
符号 & 含义\\
\midrule
$\mathcal{F}_{j,1}=(\mathsf{xtag}_{j,u}^{(1)},\ldots,\mathsf{xtag}_{j,u}^{(\ell)})$ & 非主关键词 $w_j$ 与主项条目对应的完整地址集合\\
$H_m:\{0,1\}^*\to\{0,1\}^{\lambda}$ & Merkle 开封认证哈希函数\\
$L_{j,1}^{(r)}$ & 第 $r$ 个叶子摘要，$L_{j,u}^{(r)}=H_m(0\|r\|\mathsf{xtag}_{j,1}^{(r)})$\\
$\mathsf{rt}_{j,1}$ & 地址集合 $\mathcal{F}_{j,1}$ 的 Merkle 根\\
$\sigma_{j,1}$ & $\mathcal{G}$ 对 $(I(w_j)\|\mathsf{rt}_{j,1})$ 的签名\\
$\mathsf{Auth}_{j,1}=(\mathsf{rt}_{j,1},\sigma_{j,1})$ & 地址集合的根认证对象\\
$\pi_{j,1}^{(r)}$ & 第 $r$ 个叶子到根 $\mathsf{rt}_{j,1}$ 的 Merkle 认证路径\\
$\mathsf{Open}_{j,1}$ & 采样位置的选择性开封材料\\
...
Nomos 搜索的协议执行视图以主项条目stag为索引（假设主项条目对应的关键词为$w_1$），而非以明文文件标识 $\mathsf{id}$ 为索引。服务器在枚举 $\mathsf{TSet}$ 时仅能看到条目对应的 $(\mathsf{sval},\alpha)$，客户端在解密 $\mathsf{sval}$ 后才恢复 $(\mathsf{id},\mathsf{op})$。因此，本章把地址绑定对象记为 $(j,1)$：$j\in\{2,\ldots,n\}$ 表示非主关键词索引，$stag$ 表示主项条目。客户端在本地解密后把 $(j,1)$ 解释为语义上的候选关系 $(w_j,\mathsf{id}_1)$。

地址集合 $\mathcal{F}_{j,1}$ 包含与条目 $stag$ 和关键词 $w_j$ 对应的全部 $\ell$ 个交叉标签。客户端在单次查询中仅需要验证由令牌采样位置 $\{\beta_t\}_{t\in[k]}$ 选中的子集
\begin{equation}\label{eq:commit-sampled-set}
\mathcal{A}_{j,1}^{(Q)}=\{\mathsf{xtag}_{j,1}^{(\beta_t)}\}_{t\in[k]}\text{.}
\end{equation}
本章的 Merkle 开封机制对完整集合 $\mathcal{F}_{j,1}$ 建立认证根，对采样集合 $\mathcal{A}_{j,1}^{(Q)}$ 进行选择性开封。

\section{地址绑定问题与 Merkle-open 机制设计}\label{sec:commit-attack-goal}

\subsection{位置盲化与地址替换攻击}\label{sec:commit-gap}

第\ref{chap:bf}章的 $\Pi_{\mathsf{VQ}}$ 机制在 $\mathsf{QTree}$ 上为资格检验建立了路径级认证。该机制保证的对象是位值真实性：服务器给出的 $(a,b,\pi_a)$ 若通过 $\mathsf{VfPath}$ 验证，则 $b$ 必然是版本根 $R_X^{(t)}$ 下地址 $a$ 的真实位值。该保证并不回答另一个问题：地址 $a$ 是否确实属于令牌绑定的查询侧地址集合。第\ref{chap:bf}章的定义 2 将``地址归属正确''作为绑定健全性的显式前提条件，命题~\ref{prop:vq-sound} 的安全归约在该前提下成立。本节分析该前提在 Nomos 的 OPRF 盲化架构下为何无法由客户端自行验证，从而构成真实攻击面。%TODO: bf.tex 定义 2 改用 definition 环境后替换为 \ref

Nomos 的令牌生成协议（算法~\ref{alg:sse-gentoken}）通过 OPRF 盲化保护查询隐私：客户端在令牌生成阶段只持有盲化后的 $\mathsf{xtoken}$ 材料和采样位置 $\{\beta_t\}$，不持有用于直接计算交叉标签的密钥 $K_X$。服务器在搜索阶段使用 $\alpha_u$、$\rho_j$ 与 $\mathsf{xtoken}_{j,u}^{(t)}$ 恢复采样地址
\begin{equation}\label{eq:commit-xtag-sampled}
\mathsf{xtag}_{j,u}^{(\beta_t)}\leftarrow (\mathsf{xtoken}_{j,u}^{(t)})^{\alpha_u\cdot\rho_j^{-1}}\text{,}
\end{equation}
但客户端无法独立重算这些地址。$\mathsf{QTree}$ 路径验证 $\mathsf{VfPath}(R_X^{(t)},a,b,\pi_a)$ 仅检查三元组 $(a,b,\pi_a)$ 的内部一致性——叶子摘要沿路径 $\pi_a$ 是否归约到根 $R_X^{(t)}$——而不检查地址 $a$ 是否属于令牌绑定集合 $\mathcal{A}_{j,u}^{(Q)}$。恶意服务器因此可以保留真实的 $\mathsf{QTree}$ 路径结构，同时把某个应被验证的地址替换为集合外的无关地址 $a^\star$。只要 $a^\star$ 在当前版本的 $\mathsf{QTree}$ 中确实存在且位值满足服务器期望的逻辑判定，路径验证仍会通过。

以下给出一个具体的攻击策略。设候选 $(j,u)$ 的真实判定为 $v_{j,u}^{(Q)}=0$，即存在某个采样地址 $a_0\in\mathcal{A}_{j,u}^{(Q)}$ 满足 $\mathbf{B}^{(t)}[a_0]=0$。恶意服务器选择一个集合外地址 $a^\star\notin\mathcal{A}_{j,u}^{(Q)}$，使得 $\mathbf{B}^{(t)}[a^\star]=1$——在 RBF 位数组中，多数地址的位值为 1，因此这样的 $a^\star$ 容易找到。服务器在 Negative 证据中用 $(a^\star,1,\pi_{a^\star})$ 替换 $(a_0,0,\pi_{a_0})$，并将声明判定位翻转为 $\widehat{v}_{j,u}=1$。由于 $a^\star$ 在 $\mathsf{QTree}$ 中确实存在且 $\mathbf{B}^{(t)}[a^\star]=1$，路径验证 $\mathsf{VfPath}(R_X^{(t)},a^\star,1,\pi_{a^\star})=1$ 成立。客户端在不知道 $\mathcal{A}_{j,u}^{(Q)}$ 的真实内容的情况下，无法检测到地址替换，从而接受伪造的 Positive 判定。该攻击使不满足资格条件的候选通过检验，破坏了资格检验的健全性。

\subsection{地址绑定安全目标}\label{sec:commit-goal}

地址绑定机制的目标是把第\ref{chap:bf}章中``位值真实性''的前提条件形式化为可验证对象。对查询 $Q=(w_1,\ldots,w_n)$，客户端在验证阶段根据响应中的主项条目解密结果得到条目语义映射 $u\mapsto(\mathsf{id}_u,\mathsf{op}_u)$。对每个非主关键词 $w_j$ 和条目 $u$，令 $\mathsf{ProofAddr}_{j,u}$ 表示服务器在 $\mathsf{QTree}$ 证明中实际使用的地址集合。地址绑定健全性要求：客户端仅当 $\mathsf{ProofAddr}_{j,u}$ 确属 $\mathcal{G}$ 在更新阶段认证过的采样地址集合时才接受。

安全条件采用子集关系 $\mathsf{ProofAddr}_{j,u}\subseteq\mathcal{A}_{j,u}^{(Q)}$ 而非等集关系 $\mathsf{ProofAddr}_{j,u}=\mathcal{A}_{j,u}^{(Q)}$，原因在于第\ref{chap:bf}章的双向证据结构：Positive 证据（$\widehat{v}_{j,u}=1$）必须提供全部 $k$ 个地址的证明，此时 $\mathsf{ProofAddr}_{j,u}=\mathcal{A}_{j,u}^{(Q)}$；Negative 证据（$\widehat{v}_{j,u}=0$）只需提供至少一个零位地址的反例路径，此时 $\mathsf{ProofAddr}_{j,u}\subset\mathcal{A}_{j,u}^{(Q)}$ 为真子集。子集关系统一覆盖两种证据类型。

\begin{definition}[地址绑定健全性]\label{def:addr-binding}
设 $\mathcal{A}_{j,u}^{(Q)}$ 如式~(\ref{eq:commit-sampled-set}) 所定义。安全实验 $\mathsf{Exp}_{\Pi,\mathcal{A}}^{\mathsf{AB\text{-}Sound}}(\lambda)$ 按如下方式执行：挑战者运行 $\mathsf{Setup}(1^\lambda)$ 生成密钥并初始化 $\mathsf{EDB}$、$\mathsf{MSet}$；对手 $\mathcal{A}$ 可自适应地发起 $\mathsf{Update}$ 查询（挑战者诚实执行算法~\ref{alg:commit-update} 并将更新结果交付服务器状态）与 $\mathsf{GenToken}$ 查询（挑战者诚实生成令牌 $\tau_{\mathsf{VQ}}$）；$\mathcal{A}$ 最终输出一个查询 $Q^\star$ 及对应的响应 $\rho'$。实验的胜利条件为
\begin{equation}\label{eq:addr-binding}
\mathsf{Verify}'(\rho')=1\ \land\ \exists(j,u):\mathsf{ProofAddr}_{j,u}\not\subseteq \mathcal{A}_{j,u}^{(Q^\star)}\text{.}
\end{equation}
对手优势定义为 $\mathsf{Adv}_{\Pi,\mathcal{A}}^{\mathsf{AB\text{-}Sound}}(\lambda)=\Pr[\mathsf{Exp}_{\Pi,\mathcal{A}}^{\mathsf{AB\text{-}Sound}}(\lambda)=1]$。地址绑定健全性要求：对任意 PPT 对手 $\mathcal{A}$，$\mathsf{Adv}_{\Pi,\mathcal{A}}^{\mathsf{AB\text{-}Sound}}(\lambda)\le\mathsf{negl}(\lambda)$。
\end{definition}

定义~\ref{def:addr-binding} 与第\ref{chap:bf}章的定义 2 构成互补关系。定义 2 保证``若服务器确实在正确地址上生成证据，则位值不可伪造''；定义~\ref{def:addr-binding} 保证``服务器不能把证明地址替换到认证集合之外''。两项性质共同闭合资格检验的正确性链条。该定义与标准 Merkle 承诺健全性的区别在于承诺对象：标准 Merkle 承诺保证``叶子值不可篡改''，而地址绑定健全性保证``证明所用的地址集合不可替换''。前者是后者的构造工具——Merkle 路径验证用于实现地址归属的可验证性——但安全目标不同。%TODO: bf.tex 定义 2 改用 definition 环境后替换为 \ref

以下基于上述安全目标，给出满足定义~\ref{def:addr-binding} 的 Merkle 开封构造。

\subsection{设计约束与策略选择}\label{sec:commit-design}

Nomos 原始搜索令牌仅支持服务器恢复 $k$ 个采样位置对应的交叉标签，而不支持在搜索阶段额外恢复完整的 $\ell$ 个地址。若地址绑定机制要求服务器返回完整集合 $\mathcal{F}_{j,u}$，则令牌语法、搜索代数结构和通信复杂度都将偏离第\ref{chap:bf}章的基础协议。地址绑定机制因此必须满足下述一致性约束：更新阶段可以对完整集合 $\mathcal{F}_{j,u}$ 建立认证，搜索阶段只能对采样集合 $\mathcal{A}_{j,u}^{(Q)}$ 返回证明。

在该约束下，存在两种直接但不可行的替代方案。第一种是完整集合开封方案：搜索阶段返回全部 $\ell$ 个地址及其认证路径，客户端逐一验证后确认采样子集的归属。该方案的通信开销为 $O(\ell\cdot|\mathbb{G}|)$，与 Nomos 的 $k$-采样语法不一致——基础协议的搜索通信仅依赖 $k$ 而非 $\ell$，完整开封将使地址绑定验证的通信成本超过资格检验本身。第二种是对称密钥派生方案：将 $K_X$ 的部分派生材料嵌入令牌，使客户端在本地计算 $\mathcal{A}_{j,u}^{(Q)}$ 并直接比对服务器返回的地址。该方案破坏了 Nomos 的 OPRF 盲化架构——$K_X$ 的任何派生材料一旦暴露给客户端，$\mathcal{G}$ 将无法保证令牌生成阶段的查询隐私。

Merkle 开封构造满足该约束。更新阶段，$\mathcal{G}$ 对完整集合 $\mathcal{F}_{j,u}$ 建立一棵大小为 $\ell$ 的 Merkle 树，并输出根认证对象 $\mathsf{Auth}_{j,u}=(\mathsf{rt}_{j,u},\sigma_{j,u})$。搜索阶段，服务器只对采样位置 $\{\beta_t\}_{t\in[k]}$ 返回对应叶子 $\mathsf{xtag}_{j,u}^{(\beta_t)}$ 及其路径 $\pi_{j,u}^{(\beta_t)}$。客户端验证的对象因此从``完整集合内容''变成``采样地址确属某个经授权管理方认证的完整集合''。该构造的可行性源于 Nomos 更新流程的一个结构性特征：$\mathcal{G}$ 在更新阶段已经计算了完整的 $\ell$ 个交叉标签（算法~\ref{alg:sse-update}），因此 Merkle 树构造不需要额外的密码学运算，只需在已有标签上执行 $O(\ell)$ 次哈希。搜索阶段的附加通信由 $k$ 控制而非由 $\ell$ 控制，与 Nomos 的 $k$ 位置采样搜索完全同构。

\subsection{完整集合的根认证}

对每个关系 $(w,\mathsf{id},\mathsf{op})$，$\mathcal{G}$ 在更新阶段计算全部 $\ell$ 个交叉标签。此处以单条关系为例省略 $(j,u)$ 下标，记完整集合为 $\mathcal{F}=(\mathsf{xtag}^{(1)},\ldots,\mathsf{xtag}^{(\ell)})$，并为每个投影位置构造叶子摘要
...
签名消息中显式包含关键词桶索引 $I(w)$，而非仅对根 $\mathsf{rt}$ 签名。该绑定防止跨关键词根认证对象重放：若签名仅覆盖 $\mathsf{rt}$，恶意服务器可将关键词 $w_j$ 的根认证对象用于关键词 $w_{j'}$（$j\neq j'$），只要两者恰好共享相同的 Merkle 根。包含 $I(w)$ 后，客户端在验证阶段可检查签名消息中的桶索引是否与令牌中的 $I(w_j)$ 一致，从而排除此类重放。
...
\subsection{选择性开封与验证逻辑}

对查询 $Q=(w_1,\ldots,w_n)$，客户端在令牌中保留采样位置 $\{\beta_t\}_{t\in[k]}$。服务器在搜索阶段对每个 $(j,u)$ 只返回
\begin{equation}\label{eq:commit-open}
\mathsf{Open}_{j,u}=\left\{(\beta_t,\mathsf{xtag}_{j,u}^{(\beta_t)},\pi_{j,u}^{(\beta_t)})\right\}_{t\in[k]}
\end{equation}
以及一次根认证对象 $\mathsf{Auth}_{j,u}=(\mathsf{rt}_{j,u},\sigma_{j,u})$。客户端据此执行三步检查：
\begin{enumerate}
\item 验证 $\mathsf{VfSig}(\mathsf{pk}_G,I(w_j)\|\mathsf{rt}_{j,u},\sigma_{j,u})=1$，确认根对象由 $\mathcal{G}$ 认证；
\item 对每个 $t\in[k]$ 验证
\begin{equation}\label{eq:commit-vfmpath}
\mathsf{VfMPath}\!\left(\mathsf{rt}_{j,u},\,H_m(0\|\beta_t\|\mathsf{xtag}_{j,u}^{(\beta_t)}),\,\pi_{j,u}^{(\beta_t)}\right)=1\text{,}
\end{equation}
确认采样地址属于同一完整集合；
...
三步检查的正确性可从以下角度理解。第一步的签名验证保证根 $\mathsf{rt}_{j,u}$ 由 $\mathcal{G}$ 在某次更新中输出，排除了服务器伪造根认证对象的可能。第二步的 Merkle 路径验证保证每个开封的交叉标签 $\mathsf{xtag}_{j,u}^{(\beta_t)}$ 确实是根 $\mathsf{rt}_{j,u}$ 在位置 $\beta_t$ 上的真实叶子值，排除了服务器用集合外地址冒充采样地址的可能。第三步的地址归属检查保证 $\mathsf{QTree}$ 证明中实际使用的地址与已开封的采样集合一致，将 Merkle 开封的保证传递到 $\mathsf{QTree}$ 路径验证。三步检查的组合效果是：客户端接受的每个 $\mathsf{QTree}$ 证明地址都经过 $\mathcal{G}$ 认证的 Merkle 树验证，从而闭合了第\ref{chap:bf}章定义 2 中``地址归属正确''的前提条件。

开封材料的大小由采样参数 $k$ 控制。对每个 $(j,u)$，服务器返回 $k$ 个三元组 $(\beta_t,\mathsf{xtag}_{j,u}^{(\beta_t)},\pi_{j,u}^{(\beta_t)})$ 和一个根认证对象 $(\mathsf{rt}_{j,u},\sigma_{j,u})$。每条 Merkle 路径的长度为 $O(\log\ell)$ 个哈希值，因此单个 $(j,u)$ 的开封通信为 $O(k\cdot(|\mathbb{G}|+\lambda\log\ell))$ 比特。与返回完整 $\ell$ 元开封材料的方案相比，该通信量降低了 $\ell/k$ 倍，且与 Nomos 基础搜索中每个 $(j,u)$ 使用 $k$ 个交叉令牌的通信结构一致。
...
\subsection{开销分析}\label{sec:commit-eval}
