...
\centering
\footnotesize
\caption{本文方案与 Nomos 基线的渐近开销对比。表中 $|\mathsf{Cand}|$ 为 $|\mathsf{Cand}(w_s)|$ 的简写。Nomos 基线不提供客户端验证机制（标记为``---''），本文方案的验证开销即为从零引入可验证性的全部代价。本文方案在更新与搜索阶段引入的额外开销均为对数级或线性级增量}
...
\caption{不同数据规模下的单次关系更新耗时。图中对比 Nomos 基线与本文方案在不同 $N$ 值下的更新性能。本文方案的额外开销主要来自 $\mathsf{QTree}$ 路径更新（$O(\ell\log M)$ 次哈希运算），在默认参数下约为基线的 $3.5$ 倍}
...
\caption{更新开销分解（$M=2^{22}$，$N=10^5$）。图中将本文方案的更新开销分解为三部分：Nomos 基线更新、$\mathsf{QTree}$ 路径更新与承诺计算。$\mathsf{QTree}$ 路径更新占主要额外开销，承诺计算为低阶项}
...
\caption{不同合取关键词数下的单次查询通信量（$|\mathsf{Cand}|=1000$，$k=10$）。图中对比 Nomos 基线与本文方案在不同合取关键词数 $n$ 下的通信开销。本文方案的额外通信量主要来自 $\mathsf{QTree}$ 认证路径（$O(k\log M)$）与开封材料（$O(\ell \cdot |\mathbb{G}|)$），随 $n$ 线性增长}
...
\caption{不同数据规模下的服务器端存储开销（$\ell=20$，$M=2^{22}$）。图中对比 Nomos 基线与本文方案在不同 $N$ 值下的存储需求。本文方案的额外存储主要来自 $\mathsf{QTree}$ 节点（$O(M\cdot\lambda)$）与承诺嵌入（$O(N\cdot\lambda)$），均为线性增量}
...
更新阶段的瓶颈相对较轻。$O(\ell\log M)$ 的路径更新开销与 $\ell$ 和 $\log M$ 两个因子相关。在默认参数 $\ell=20$，$M=2^{22}$ 下，路径更新包含约 $20 \times 22 = 440$ 次哈希运算，单次更新耗时约 $35$ $\mu$s，与基线 Nomos 的单次更新（~10 $\mu$s）相比增加约 $3.5$ 倍。该开销在可接受范围内，但高频更新场景下可能成为瓶颈。
